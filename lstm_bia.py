...
class LSTMBiA(nn.Module):
    def __init__(self, embedding_dim, vocab_num, hidden_dim, batch_size, dropout, num_layer, model_num_layer, bi_direction=True, pretrained_weight=None):
        ...
        self.conv_lstm = nn.LSTM(hidden_dim + 1, self.hidden_dim, num_layers=self.conv_num_layer, bidirectional=self.bi_direction)
        self.conv_hidden = self.init_hidden(self.batch_size, self.conv_num_layer)

        self.similarity = nn.Linear(hidden_dim * 3, 1)

        self.model_lstm = nn.LSTM(hidden_dim, self.hidden_dim, dropout=dropout, num_layers=self.model_num_layer, bidirectional=self.bi_direction)
    ...
    def forward(self, target_conv, user_history):
        ...
        for conv in target_conv:
            ...
            if torch.cuda.is_available():  # run in GPU
                ...
                sent_reps = self.sent_lstm(self.word_embedding(conv[:turn_num, 1:].cuda()), torch.LongTensor(sent_lens).cuda())
            else:
                sent_reps = self.sent_lstm(self.word_embedding(conv[:turn_num, 1:]), torch.LongTensor(sent_lens))
        ...
        user_history = torch.cat([one_his.unsqueeze(0) for one_his in user_history], dim=0)
        sorted_user_history = user_history[sorted_his_indices]
        sorted_user_history = sorted_user_history[:, :, 1:]  # leave out the conv IDs
        his_out = []
        his_num = 0
